...
export class GoldenCookieModel {
  ...
  goldenVisibility(shimmer: GameShimmer): GoldenVisibility {
    ...
    const curve = 1 - Math.pow(lifeRatio * 2 - 1, 4);
    const ready = progress >= 0.5 || curve >= this.getGoldenFadeThreshold();

    return { curve, progress, ready };
  }

  /** Classifies all live golden shimmers: good (clickable), pending (still fading in, good
   * type), wrath (never clicked). Also remembers when each good cookie first became ready
  ...
  getGoldenShimmers(): GoldenShimmers {
    ...
    for (const shimmer of this.game.getShimmers()) {
      if (!shimmer || shimmer.type !== 'golden' || shimmer.popped || !shimmer.l || !shimmer.l.isConnected) {
        continue;
      }
